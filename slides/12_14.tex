...
\hypersetup{
  pdftitle={\textbackslash Box(\textbackslash Box A \textbackslash rightarrow B) \textbackslash vee \textbackslash Box(\textbackslash Box B \textbackslash rightarrow A) (in S4)},
...
\title{\(\Box(\Box A \rightarrow B) \vee \Box(\Box B \rightarrow A)\) (in S4)}
...
\item
  Note we have no special rules for 4 or T triggered yet, because no
  true \(\Box\) or false \(\Diamond\), but that's about to change.
\end{itemize}
...
\begin{frame}{\(\Box(\Box A \rightarrow B) \vee \Box(\Box B \rightarrow A)\)}
\protect\hypertarget{boxbox-a-rightarrow-b-vee-boxbox-b-rightarrow-a-5}{}
...
But now we've applied all the rules, and the tableau doesn't close. So
not a theorem.
